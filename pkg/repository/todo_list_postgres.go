...
func (r *TodoListPostgres) Delete(userId, listId int) (int, error) {
	queyr := fmt.Sprintf("DELETE FROM %s tl USING %s ul WHERE tl.id = ul.list_id AND ul.user_id = $1 AND ul.list_id = $2",
		todoListsTable, usersListsTable)
